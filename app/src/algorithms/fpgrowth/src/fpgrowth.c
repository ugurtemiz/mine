...
#define VERSION     "version 6.10 (2016.03.01)        " \
                    "(c) 2004-2016   Christian Borgelt"
...
{                               /* --- main function */
  ...
  int     prefmt   = 1;         /* flag for pre-formatting integers */
  PATSPEC *psp;                 /* collected pattern spectrum */
  ITEM    m;                    /* number of items */
  TID     n, r;                 /* number of transactions */
  SUPP    w;                    /* total transaction weight */
  #ifndef QUIET                 /* if not quiet version */
  clock_t t;                    /* timer for measurements */
  ...
  #endif  /* #ifndef QUIET */
  /* free option characters: jy [A-Z]\[ACFINPRSZ] */
  ...
  thresh /= 100.0;              /* scale the evaluation threshold */
  ...
  smax = (smax >= 0) ? smax/100.0 *(double)w *(1+DBL_EPSILON) : -smax;
  ...
  r = (prefmt) ? (TID)ib_maxfrq(ibase) : -1;
  if (isr_prefmt(report, (TID)smin, r) != 0)
    error(E_NOMEM);             /* pre-format integer numbers */
